...
//!
//! Mirrors what op-succinct does in-zkVM at
//! `utils/client/src/oracle/blob_provider.rs:28-37` (using `kzg-rs` there) and
//! the host-side `c_kzg` calls at
//! `utils/host/src/witness_generation/online_blob_store.rs:46-48`.
//!
//! The check answers: "given these blob bytes and the (commitment, proof) the
//! beacon node returned, does the proof verify against the trusted KZG setup?"
//! Combined with the `kzg_to_versioned_hash(commitment) == versioned_hash`
//! identity (the L1 transaction signed that hash), this rules out the beacon
//! node lying about blob content.
