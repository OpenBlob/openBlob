...
//!
//! Bootstraps once against `/eth/v1/beacon/genesis` and `/eth/v1/config/spec`
//! to learn `genesis_time` + `SECONDS_PER_SLOT`, then resolves a slot from an
//! L1 block timestamp and fetches its blob sidecars.
...
/// because the beacon node is trusted and downstream we only sanity-check the
/// commitment ↔ versioned-hash relationship.
#[derive(Debug, Deserialize)]
struct RawSidecar {
    #[serde(deserialize_with = "de_str_u64")]
    index: u64,
